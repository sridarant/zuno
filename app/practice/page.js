...
setQ(next);setPhase("intro");setMsg("Wait...");
...
if(correct){setMood("correct");setMsg("That was clean 😏");setTimeout(loadQ,1500);}
else{setMood("wrong");setMsg("Try that again… properly.");}
};
if(!q) return null;
return(<main style={{padding:20,height:'100vh',position:'relative'}}>
<ZunoCharacter mood={mood}/>
{phase==="question" && (<><div style={{background:'#0f172a',padding:24,borderRadius:20}}><h2>{q.q}</h2></div>
<button onClick={()=>setPhase("commit")} style={btn}>I’ve written it</button></>)}
{phase==="commit" && (<><input value={input} onChange={(e)=>setInput(e.target.value)} style={inputStyle}/>
